import device from 'components/OTPInputView/helpers/device';
import React, {
    Component,
    useEffect,
    useImperativeHandle,
...
function CCarouselVer(props: any, ref: any) {
    const {
        data = [],
        style = {},
    ...
    function setScrollHandler() {
        handleOnScrollRef.current = Animated.event(
            [{ nativeEvent: { contentOffset: { y: xOffsetRef.current } } }],
            {
                useNativeDriver: true,
                listener: (event: any) => {
                    scrollXRef.current = event.nativeEvent.contentOffset.y;
                }
            }
        );
    }
    ...
    function getItemTotalMarginBothSide() {
        const compensatorOfSeparatorByScaleEffect = (1 - inActiveScale) * itemHeight;
    ...
    function getAnimatedOffset(index: number) {
        if (isFirstItem(index)) {
            return halfItemHeight;
        }
        if (isLastItem(index)) {
            return containerHeight - halfItemHeight;
        }
        return halfContainerHeight;
    }
    ...
    function getStartPontInterpolate(index: number, midPoint: number) {
        ...
        return midPoint - itemHeight - itemTotalMarginBothSide;
    }
    ...
    function getEndPointInterpolate(index: number, midPoint: number) {
        ...
        if (index === dataLength - 2) {
            return (
                (dataLength - 1) * (itemHeight + itemTotalMarginBothSide) +
                itemHeight -
                containerHeight
            );
        }
...
export default forwardRef(CCarouselVer);
